    ...
    >
      <div className="grid gap-5 md:grid-cols-2 xl:grid-cols-4">
        {stats.map((item, index) => (
          <article
            key={item.label}
            className="rounded-[30px] border border-app-border bg-white p-6 shadow-card transition-all duration-200 ease-apple hover:-translate-y-0.5 hover:shadow-float"
          >
            <div className="flex items-center justify-between">
              <span className="text-sm text-app-muted">{item.label}</span>
              <span className="flex h-10 w-10 items-center justify-center rounded-2xl bg-[#f8f8fa] text-sm font-medium text-app-text">
                0{index + 1}
              </span>
            </div>
      ...
      <div className="grid gap-6 xl:grid-cols-[1.1fr_0.9fr]">
        <article className="rounded-[32px] border border-app-border bg-white p-6 shadow-card">
          <div className="flex items-center justify-between">
            <div>
            ...
            <button className="rounded-xl border border-app-border bg-white px-4 py-2.5 text-sm font-medium text-app-text transition hover:bg-[#fafafb]">
              Ver detalle
            </button>
          </div>

          <div className="mt-8 grid h-[300px] grid-cols-7 items-end gap-3 rounded-[28px] bg-[#fafafb] p-6">
            {[42, 58, 46, 72, 64, 88, 76].map((value, index) => (
              <div key={value} className="flex flex-col items-center gap-3">
                <div className="flex h-56 items-end">
                  <div
                    className="w-10 rounded-full bg-gradient-to-t from-[#111827] to-[#4b5563] transition-all duration-300 ease-apple hover:opacity-85"
        ...
        <article className="rounded-[32px] border border-app-border bg-white p-6 shadow-card">
          ...
          <div className="mt-6 space-y-3">
            {activity.map((item, index) => (
              <div
                key={item}
                className="flex items-start gap-4 rounded-2xl bg-[#fafafb] px-4 py-4"
              >
      ...
      <div className="grid gap-6 xl:grid-cols-[1.15fr_0.85fr]">
        <article className="rounded-[32px] border border-app-border bg-white p-6 shadow-card">
          <div className="flex items-center justify-between">
            ...
            <span className="rounded-full bg-blue-50 px-3 py-1 text-xs font-medium text-app-accent">
              Hoy
            </span>
          </div>

          <div className="mt-6 overflow-hidden rounded-3xl border border-app-border">
            <div className="grid grid-cols-3 bg-[#fafafb] px-5 py-4 text-xs font-semibold uppercase tracking-[0.18em] text-app-muted">
              <span>Producto</span>
              <span>Estado</span>
            ...
            {tableRows.map((row) => (
              <div
                key={row.item}
                className="grid grid-cols-3 items-center border-t border-app-border px-5 py-4 text-sm transition-colors hover:bg-[#fafafb]"
              >
        ...
        <article className="rounded-[32px] border border-app-border bg-white p-6 shadow-card">
          ...
          <div className="mt-6 space-y-4">
            {[
              ...
              <div
                key={title}
                className="rounded-2xl border border-app-border bg-[#fafafb] p-4"
              >
